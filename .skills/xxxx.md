...
- Viewing a 6-day structured daily itinerary with timeline
...
└── src/
    ...
    └── components/
        ...
        ├── DailyItineraryView.tsx # Timeline view with itinerary items
...
- **`defaultDayPlans`** — Array of 4 `DayPlan` objects for days 12–15 with full itinerary items and GPS coordinates; the Singapore extension is documented in the map snapshot below
- **`defaultExpenses`** — Array of 13 seed `Expense` objects (IDs: "e-1" to "e-13") covering the core Malaysia days
...
- Contains: hero data, legend items, budget summary cards, alert box, tip cards, day sections, footer text, plus the Singapore extension snapshot below
...
- `coordinateHints` — Array of 52 `{ match: string[], coords }` objects for fuzzy name→lat/lng resolution
...
### Singapore Extension Snapshot
The itinerary and map layers now document the Singapore leg in addition to Kuala Lumpur and Melaka. The new trip window includes:
- July 11: flight departure and arrival day
- July 15: KL to Singapore travel day, hotel drop-off at Hotel Classic by Venue, then Geylang Serai, Little India, Haji Lane, Gardens by the Bay, and Marina Bay night views
- July 16: Singapore morning walk starting from Hotel Classic by Venue, then Merlion Park, Raffles Place, Joo Chiat/Koon Seng Road, Chinatown, Jewel Changi, and Changi Airport departure

New Singapore map coordinate hints include Hotel Classic by Venue, Paya Lebar MRT, Geylang Serai Market, Little India, Sri Veeramakaliamman Temple, House of Tan Teng Niah, Haji Lane, Sultan Mosque, Gardens by the Bay, Merlion Park, Raffles Place, Koon Seng Road, Chinatown Singapore, Chinatown Complex, Jewel Changi, and Changi Airport.
